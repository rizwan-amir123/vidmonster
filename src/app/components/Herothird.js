const Herothird = ({src}) => {
    const creators_one = ["Markplier", "Shane Dawson", "Dude Perfect", "Smosh", "Preston"]
    const creators_two = ["Preston", "David Dorbik",
    "JuegaGerman", "ElRubiusOMG"];
    return (
    <section className="bg-gradient-to-r from-[#4f19bb] to-[#4adb0d] px-5 text-white py-20">
      ...
      <div key={index} className="overflow-hidden w-1/4 bg-red-400 cursor-pointer rounded-xl relative group"
        >
            <div
...
{creators_two.map((creator, index) => (
      <div key={index} className="overflow-hidden w-1/4 h-1/8 bg-red-400 cursor-pointer rounded-xl relative group"
        >
            <div
                className="rounded-xl z-50 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out cursor-pointer absolute from-black/80 to-transparent bg-gradient-to-t inset-x-0 -bottom-2 pt-30 text-white flex items-end"
            >
